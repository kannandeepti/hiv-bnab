...
Used to create Figure 5 of the main text.
"""
...
def plot_epitope_frequencies_violin(lineage_data, ep_boundaries, time_points, title):
    ...
    for idx, t in enumerate(time_points):
        ...
        sns.violinplot(
            data=[epitope_frequencies[:, i] for i in range(n_ep)],
            palette=epitope_colors(n_ep),
            ax=axs[idx],
        )
...
def plot_time_clone_became_dominant(
    ...
    for i in range(n_ep):
        ...
        ax.plot(
            time[0:200],
            ep_dom_clone_dynamics[0:200, traj_inds],
            lw=0.3,
            color=colors[i],
            label="_nolegend_",
        )
        ...
        ax.plot(
            time[0:200],
            mean_frequencies[0:200],
            color=darken_color(colors[i], 0.1),
            label=f"ep {i+1}",
        )
...
def make_lineage_plots(param_dir, title):
    ...
    epitope_frequencies = plot_epitope_frequencies_violin(
        lineage["gc"], lineage["ep_boundaries"], time_points_to_plot, title
    )
    ...
    time_indices_egc = [
        ...
        99,
        199,
    ]  # Replace with your specific time points
    plot_gc_pie_charts(
        lineage["gc"],
        lineage["ep_boundaries"],
        time_indices_gc,
        replicate_idxs,
        gc_idxs,
        title,
