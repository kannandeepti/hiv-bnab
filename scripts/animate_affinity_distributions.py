...
def animate_Bcell_affinity_distributions(aff_distr, field, title):
    ...
    def update(frame):
        ...
        for i in range(n_ep):
            ax.plot(
                aff_distr["affinity_bins"],
                aff_distr[field][t_idx, :, i, :].mean(axis=0),
                lw=2,
                label=f"ep {i+1}",
                color=colors[i],
            )
            ax.fill_between(
                aff_distr["affinity_bins"],
                aff_distr[field][t_idx, :, i, :].min(axis=0),
                aff_distr[field][t_idx, :, i, :].max(axis=0),
                color=colors[i],
                alpha=0.3,
                label=None,
            )
