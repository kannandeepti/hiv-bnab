...
Simulation class
================
The `Simulation` class is the workhorse for simulating the humoral immune response to
bnAb therapy. The `run()` function runs an entire simulation of a persistent humoral
response for `simulation_time` days and saves a history.pkl file containing the numbers
of B cells in each population (naive, GC, memory GC, memory EGC, plasma GC, plasma
...
class Simulation(Parameters):
    ...
    Attributes:
        ...
        parallel_run_idx: Index for parallel runs, used to distinguish output files and
            results.
        data_dir: Directory for simulation outputs.
        history_path: Path to the history pickle file.
        sim_path: Path to the simulation pickle file.
        parameter_json_path: Path to the parameter JSON file.
    """

    def __init__(
        self, updated_params_file: str | None = None, parallel_run_idx: int = 0
    ):
        """
        Initialize a Simulation instance, setting up all simulation parameters and random seed.

        This constructor loads all default parameters from the Parameters base class. If
        `updated_params_file` is provided, parameters specified in that file will override
        the defaults. The random seed for NumPy is set using the `seed` attribute to ensure
        reproducibility.

        All other classes inheriting from Parameters should also receive the `updated_params_file`
        argument to maintain consistent parameter updates throughout the simulation components.

        Args:
            updated_params_file (str, optional): Path to a JSON file containing updated parameters.
            parallel_run_idx (int): Index for parallel runs, used to distinguish output files and results.
        """
        super().__init__()
    ...
    def reset_history(self) -> None:
        ...
        * ``'ep_to_lineage'`` (list[int]):
        ...
        self.history = {
            ...
            "gc": {
                ...
                "num_by_lineage": np.zeros(
                    (self.n_history_timepoints, self.n_gc, self.n_naive_precursors)
                ),
            },
            "conc": {
                "ic_fdc_conc": np.zeros((self.n_history_timepoints, self.n_ep)),
                "ic_fdc_eff_conc": np.zeros((self.n_history_timepoints, self.n_ep)),
                "ab_conc": np.zeros(
                    (
                        self.n_history_timepoints,
                        self.n_ep,
            ...
            "plasma_gc": {
                ...
                "num_by_lineage": np.zeros(
                    (self.n_history_timepoints, self.n_naive_precursors)
                ),
            },
        }
        ...
        self.history["ep_to_lineage"] = (
            []
        )  # store the lineage index boundaries separating naive cells that target different epitopes
    ...
    def create_file_paths(self) -> None:
        ...
        * **parameter_json_path (Path)** – Location of the exported parameter
        ...
        self.data_dir = (
            Path(self.experiment_dir) / f"{date_time}_{self.parallel_run_idx}"
        )
        ...
        for ep in range(self.n_ep):
            for j, fitness in enumerate(self.fitness_array):
                ...
                naive_bcells.variant_affinities[
                    idx:idx_new, np.array(self.naive_high_affinity_variants)
                ] = fitness
    ...
    def create_populations(self) -> None:
        ...
        Attributes:
            ...
            naive_cells_entry (List[Bcells]):
                Per-GC counters used to log how many naive or memory cells entered each GC in
        ...
        self.naive_cells_entry = [
            copy.deepcopy(self.dummy_bcells) for _ in range(self.n_gc)
        ]
        self.naive_bcells = [
    ...
    def run_gc(self, gc_idx: int) -> None:
        ...
        *  GC Bcells are selected based on the effective antigen concentration
        (``self.ag_eff_conc``) and available T-cell help
        (``self.n_tfh_gc``).
        ...
        *  Non-exported cells are returned to the GC’s main compartment
        ...
        """
        seeding_bcells = self.naive_bcells[gc_idx].get_seeding_bcells(
            self.ag_eff_conc, self.seeding_tcells_gc
        )
        ...
        # Note: daughter bcells is a copy. The parents remain in the GC (unmutated)
    ...
    def run_egc(self) -> None:
        ...
        *   Append memory cells back into ``self.memory_egc_bcells``
        ...
        Returns:
            None
        """
        seeding_bcells = self.memory_gc_bcells.get_seeding_bcells(
            self.ag_eff_conc, self.seeding_tcells_egc
        )
        ...
        if self.egc_recycle_daughter:
            # recyle one of the two copies of birthed cells into the EGC (same as GC recycling logic)
    ...
    def update_history(self) -> None:
        ...
        def _fill_history_nums(
            ...
            """Fill num_above_aff, num_in_aff, and num_by_lineage in history.
        ...
        for gc_idx, bcells in enumerate(self.naive_cells_entry):
            self.history["gc_entry"]["total_num"][history_idx, gc_idx] = (
                self.naive_cells_entry[gc_idx].get_total_num()
            )
        ...
        # Concentrations
        self.history["conc"]["ic_fdc_conc"][
            history_idx
        ] = self.concentrations.ic_fdc_conc
    ...
    def run_timestep(self) -> None:
        ...
        self.concentrations.update_concentrations(
            self.current_time, self.plasma_gc_bcells, self.plasma_egc_bcells
        )
    ...
    def print_bcell_populations(self) -> None:
        ...
        print(
            "Number of plasma EGC bcells: " + f"{self.plasma_egc_bcells.lineage.size}"
        )
        print(
            "Number of memory EGC bcells: " + f"{self.memory_egc_bcells.lineage.size}"
        )
