...
This module defines the `Parameters` class, which contains all simulation parameters.
...
class Parameters:
    ...
    n_residues: int = 80
    """
    The number of residues in a B cell receptor.
    """
    ...
    """ Fixed total IC-FDC concentration (nM). """
    ...
    """
    The half-life of memory B cells (days).
    """

    memory_to_gc_fraction: float = 0.0
    """
    The fraction of memory B cells that can reenter germinal centers.
    """
    ...
    """
    The number of times a naive B cell divides upon seeding a GC.
    """

    mutation_pdf: tuple[float] = (3.1, 1.2, 3.08)
    """
    The parameters for the distribution of binding affinity changes (dE) upon mutation.
    """

    Esat: float = 10.0
    """
    Affinity saturation level [-log_10(M)]. If energy is above this value, it is clipped.
    """

    E0: float = 6
    """
    Initial affinity level [-log_10(M)].
    """

    E1hs: tuple = (7, 6.6, 6.6)
    """
    Energy levels for geometric distribution for each epitope [-log_10(M)].
    """

    n_class_bins: int = 11
    """
    The number of affinity bins that naive B cells are sorted into.
    """

    fitness_array: tuple[float] = tuple(np.linspace(E0, E0 + 2, n_class_bins))
    """
    Affinity bins (right edge) for naive B cells.
    """
    ...
    max_affinity: float = 16.0
    """
    The maximum affinity allowed for a B cell [-log_10(M)]. If affinity is above this value, an error is raised.
    """

    history_affinity_thresholds: tuple[float] = (6.0, 7.0, 8.0, 9.0)
    """
    Affinity thresholds for counting cells for the simulation history [-log_10(M)].
    """

    affinity_bins: tuple[float] = tuple(np.arange(E0, 16.2, 0.2))
    """
    Affinity bins (right edge) for GC/EGC B cells for simulation history [-log_10(M)].
    """
    ...
    """
    Fractions of naive B cells targeting each epitope.
    """

    stringency: float = 0.5
    """
    Stringency of selection, K, for Ag capture.
    """
    ...
    """
    Conversion factor from log_10(nM) to log_10(M), i.e. log_10(nM) - 9 = log_10(M)
    """
    ...
    k_ig: float = 2.5e-5
    """
    Rate of IgG production (nM/day per PC).
    """

    seeding_tcells_gc: float = 10
    """
    Tcell amount for seeding GCs (constant).
    """

    seeding_tcells_egc: float = 10
    """
    Tcell amount for seeding single EGC (constant).
    """
    ...
    mutate_start_time: float = 6.0
    """
    Time until mutations are turned on (days).
    """
    ...
    def overlap_matrix(self) -> tuple:
        ...
        if self.epitope_overlap > 0:
            if (
                self.n_ag == 2
                and self.n_variable_epitopes == 1
                and self.n_conserved_epitopes == 1
            ):
                mat[0:2, 2] = self.epitope_overlap
                mat[2, 0:2] = self.epitope_overlap
            elif (
                self.n_ag == 2
                and self.n_variable_epitopes == 0
                and self.n_conserved_epitopes == 3
            ):
                mat[0, 1:] = self.epitope_overlap
                mat[1:, 0] = self.epitope_overlap
            else:
                raise ValueError(
                    f"Epitope overlap not implemented for n_ag={self.n_ag}, n_conserved={self.n_conserved_epitopes},"
    ...
    def epitope_covariance_matrix(self) -> np.ndarray:
        ...
        for ep_idx in range(self.n_ep):
            sigma[ep_idx] = np.array([[1.0]])
        return sigma

    @property
    def naive_bcells_arr(self) -> np.ndarray:
        """Get number of naive B cells in each germline affinity bin.

        The first bin is between affinities of 6 and 6.2, the second bin
        ...
        #TODO: reference equations in STAR methods for this
        ...
        for ep in range(self.n_ep):
            if max_classes[ep] > 1:
                func = lambda x: self.n_naive_precursors - (
                    x ** max_classes[ep] - 1
                ) / (x - 1)
        ...
        for ep in range(self.n_ep):
            ...
            if max_classes[ep] > 1:
                naive_bcells_arr[ep, : self.n_class_bins] = self.naive_target_fractions[
                    ep
                ] * geo_dist_slopes[ep] ** (
                ...
                naive_bcells_arr[ep, 0] = (
                    self.naive_target_fractions[ep] * self.n_naive_precursors
                )
    ...
    def update_parameters_from_file(self, file_path: str | None = None) -> None:
        """Update parameter from default by reading a json file, if file is not None.
            ...
            for key, value in updated_params.items():
                ...
                if not hasattr(self, key):
                    raise AttributeError(
                        f"The parameter '{key}' is not a valid attribute of the Parameters class."
                    )
                ...
                if expected_type:
                    try:
                        ...
                        raise TypeError(
                            f"Cannot cast value '{value}' to type {expected_type} for attribute '{key}': {e}"
                        )
